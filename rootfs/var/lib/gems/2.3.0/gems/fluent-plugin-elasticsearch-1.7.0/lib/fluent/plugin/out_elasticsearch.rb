...
class Fluent::ElasticsearchOutput < Fluent::BufferedOutput
  ...
  def write(chunk)
    ...
    chunk.msgpack_each do |tag, time, record|
      ...
      dt = get_time(record, time)
      target_index = get_target_index(record, dt)
      ...
      log = record["log"]
      record["log"] = log.gsub(/\e\[\d*m{1}/, "").lstrip if log
  ...
  # Get the target_index
  # If we have passed in a target_index_key we should use that value first.
  # --- We should also check to see if the user wants this in logstash format meaning we need to append the date to the end
  # Next if the user only wants this in logstash format and hasn't provided a target_index_key, then use the logstash prefix
  # Last just use the index_name that was passed in to the plugin at configuration.
  def get_target_index(record, dt)
    target_index_parent, target_index_child_key = get_parent_of(record, @target_index_key)
    if target_index_parent && target_index_parent[target_index_child_key]
      target_index = target_index_parent.delete(target_index_child_key)
      if @logstash_format
        target_index = "#{target_index}-#{dt.strftime(@logstash_dateformat)}"
      end
    elsif @logstash_format && !@target_index_key
